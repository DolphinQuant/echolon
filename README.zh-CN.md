...
让 Claude Code 或 Cursor 直接用 `backtrader` 或 `vectorbt` 写过回测的人,大概都见过同一种结果:Agent 把指标名编了一个、回调函数签名瞎猜一个、异常一抛就被吞了。Echolon 就是冲着这几种翻车模式做的:错误码稳定,配置全是带类型的 Pydantic 模型,包里还自带一个 MCP 服务器,Agent 调的是结构化工具,而不是对着一堆文字描述硬猜。
