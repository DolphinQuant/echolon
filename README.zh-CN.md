...
`pip install echolon` 之后,`echolon-mcp` 命令已在你的 `PATH` 上。在你的 Agent 运行时里注册一次即可:
...
**Claude Code 用户须知**:`-s user`(即 `--scope user`)把注册写入用户级配置,在你**所有项目**里都能用;省略它的话 Claude Code 默认是 `local` 作用域,只对当前项目目录生效。命令里的 `--` 用来把注册名(`echolon`)与启动命令(`echolon-mcp`)分开。注册一次后,运行 `claude mcp list` 应能看到 `echolon` 作为已连接的 stdio 服务器。**重启 Claude Code 会话**才能加载新工具(前缀为 `mcp__echolon__*`)。

Agent 的入门导引是 [`llms.txt`](./echolon/llms.txt) — 让你的 Agent 看一次,它就知道其他东西去哪儿找。(`echolon init` 与 `echolon hello` 也会把它的副本写到生成的 workspace 根目录里,Agent 即便没有 MCP 也能在本地找到。)
...
| LLM 入门导引 | [`llms.txt`](./echolon/llms.txt)(`echolon init`/`hello` 还会把副本写到 workspace 根目录) |
