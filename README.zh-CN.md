...
**Claude Code 用户须知**:命令里的 `--` 用来把注册名(`echolon`)与启动命令(`echolon-mcp`)分开。注册一次后,运行 `claude mcp list` 应能看到 `echolon` 作为已连接的 stdio 服务器。**重启 Claude Code 会话**才能加载新工具(前缀为 `mcp__echolon__*`)。
