"""Tests for `echolon init-strategy` CLI."""
...
def test_init_minimal_creates_all_files(tmp_path):
    ...
    result = runner.invoke(app, ["init-strategy", str(out), "--template", "minimal"])
...
def test_init_momentum(tmp_path):
    out = tmp_path / "m"
    result = runner.invoke(app, ["init-strategy", str(out), "--template", "momentum_breakout"])
    assert result.exit_code == 0
...
def test_init_rsi(tmp_path):
    out = tmp_path / "r"
    result = runner.invoke(app, ["init-strategy", str(out), "--template", "rsi_mean_reversion"])
    assert result.exit_code == 0


def test_init_unknown_template_fails(tmp_path):
    out = tmp_path / "bad"
    result = runner.invoke(app, ["init-strategy", str(out), "--template", "nope"])
    assert result.exit_code != 0
...
def test_all_templates_pass_validation(tmp_path):
    for name in ("minimal", "momentum_breakout", "rsi_mean_reversion"):
        out = tmp_path / name
        result = runner.invoke(app, ["init-strategy", str(out), "--template", name])
        assert result.exit_code == 0
