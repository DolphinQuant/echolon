...
def test_migrate_rewrites_simple_import(tmp_path):
    ...
    assert "from echolon.backtest import BacktestRunner" in (tmp_path / "a.py").read_text()
...
def test_migrate_handles_import_as(tmp_path):
    ...
    result = runner.invoke(app, ["migrate", str(tmp_path)])
    assert result.exit_code == 0
    assert "import echolon.backtest as be" in (tmp_path / "a.py").read_text()
...
def test_migrate_skips_non_py_files(tmp_path):
    ...
    result = runner.invoke(app, ["migrate", str(tmp_path)])
    assert result.exit_code == 0
    assert "from echolon.quant_engine.backtest" in (tmp_path / "file.txt").read_text()
...
def test_migrate_rewrites_common_mappings(tmp_path):
    ...
    (tmp_path / "test.py").write_text(src_lines)

    result = runner.invoke(app, ["migrate", str(tmp_path)])
    assert result.exit_code == 0
    assert (tmp_path / "test.py").read_text() == expected_lines
