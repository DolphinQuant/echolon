...
- **`BT-001` (from `echolon.errors`)** — raised by `_wrap_on_bar_exception` when the platform-agnostic strategy's `on_bar()` Template Method raises (typically from the strategy's `_execute_bar()` override, or from any entry/exit/risk/sizer component invoked by it). Context includes `bar_index`, `trading_date`, `contract`, `position_size`, `file` (strategy module), and `exception_repr`. See `docs/errors/BT-001.md`.
