...
from config.quant_engine import INDICATOR_PERIOD_CAPS, INTRADAY_INDICATOR_PERIOD_CAPS
...
'''
    ...
    def _detect_crossover_pairs(self) -> List[Tuple[str, str, str, bool]]:
        """
...
def generate_strategy_params(output_path: str = None, frequency: str = "interday") -> Tuple[bool, str]:
    """
    Main entry point for generating strategy_params.py

    Args:
        output_path: Path to write strategy_params.py.
                    If None, uses default location.
        frequency: Trading frequency - "interday" (daily bars) or "intraday" (sub-daily bars).
                  Affects which period caps are used.
    ...
    try:
        print()
        print("="*70)
        print("🚀 STRATEGY PARAMETERS GENERATOR")
        print("="*70)
        print()
        print(f"Frequency: {frequency.upper()}")
        print("Mode: AUTO-CLAMPING ENABLED")
        print("  - Period parameters will be automatically clamped to indicator caps")

        if frequency == "intraday":
            print("  - TEMA/TRIX/ADXR: max ≤ 500 bars")
            print("  - ADX/DEMA: max ≤ 750 bars")
            print("  - Standard indicators: max ≤ 1000 bars")
            print("  NOTE: Period values are in BARS (not days) for intraday")
        else:
            print("  - TEMA/TRIX/ADXR: max ≤ 62 days")
            print("  - ADX/DEMA: max ≤ 93 days")
            print("  - Standard indicators: max ≤ 180 days")

        print()
        print("Source: params_to_optimize.json")
        print("Output: strategy_params.py")
        print()

        if output_path is None:
            # Default: generate in same directory as this script (platform_agnostic/)
            output_path = Path(__file__).parent.parent / 'platform_agnostic'/ 'strategy_params.py'

        generator = StrategyParamsGenerator(frequency=frequency)
        generator.write_to_file(str(output_path))

        # Print summary of any corrections made
        generator.print_correction_summary()

        return True, f"Successfully generated {output_path}"
...
if __name__ == "__main__":
    ...
    success, message = generate_strategy_params(output_path)
