...
- Market data download and retrieval (xtdata API)
...
from modules.data_pipeline.loaders.calendar_loader import is_night_market_open
...
class MiniQMTClient:
    ...
    def reconnect(self) -> bool:
        """Attempt to reconnect to miniQMT with retry loop."""
        logger.info("Attempting to reconnect to miniQMT...")
        ...
        for attempt in range(self.max_reconnect_attempts):
            try:
                ...
                logger.error(
                    "Reconnection attempt %d error: %s", attempt + 1, exc
                )
                time.sleep(self.reconnect_delay)
    ...
    def download_history_data(
        ...
        """
        Download historical market data via xtdata API.

        Args:
            symbols: Symbol or list of symbols to download.
            period: Data period ('1m', '5m', '15m', '30m', '1h', '1d', etc.).
            start_time: Start time in 'YYYYMMDD' format.
            end_time: End time in 'YYYYMMDD' format.
            count: Number of bars to download (-1 for all).
    ...
    def get_market_data(
        ...
        """
        Get market data from local cache or download if needed.

        Uses ``xtdata.get_market_data_ex`` under the hood.
        ...
        Returns:
            Dictionary keyed by symbol with DataFrames, or None on error.
        """
        try:
            if isinstance(symbols, str):
                symbols = [symbols]
            ...
            with self.data_lock:
                for sym in symbols:
                    self.market_data_cache[f"{sym}_{period}"] = market_data
    ...
    def extract_dataframe_from_data(
        ...
        """
        Extract DataFrame for a specific symbol from market data dictionary.

        Args:
            market_data: Dictionary returned by get_market_data().
        ...
        try:
            ...
            if symbol not in market_data:
                logger.error(
                    "Symbol %s not found in data. Available symbols: %s",
                    symbol,
                    list(market_data.keys()),
                )
                return None
            ...
            try:
                # Get timestamps from the first available field
                ...
                logger.error("Failed to parse data for %s: %s", symbol, parse_error)
                return None
    ...
    def get_trading_calendar(
        ...
        """
        Get trading calendar from miniQMT.

        Args:
            market: Market identifier (e.g. 'SH', 'SZ').
            start_time: Start time in 'YYYYMMDD' format.
            end_time: End time in 'YYYYMMDD' format.
        ...
        """
        try:
            trading_calendar = xtdata.get_trading_calendar(
                market, start_time=start_time, end_time=end_time
            )
            return trading_calendar
        except Exception as exc:
            logger.error("Failed to get trading calendar: %s", exc)
            return []
    ...
    def get_stock_list_in_sector(self, sector: str) -> List[str]:
        ...
        try:
            ...
            logger.error("Failed to get stock list in sector %s: %s", sector, exc)
            return []
    ...
    def subscribe_realtime_data(self, symbol: str, period: str = "1d") -> bool:
        """
        Subscribe to real-time trading data for a futures symbol.

        Args:
            symbol: Symbol to subscribe to (e.g. 'al2507.SF').
            period: Data period ('1d', '1m', '5m', etc.).
        ...
        try:
            with self.subscription_lock:
                if symbol in self.subscribed_symbols:
                    logger.info("Already subscribed to %s", symbol)
                    return True

                logger.info("Subscribing to real-time data for %s", symbol)
                logger.info(
                    "stock_code: %s, period: %s, period_type: %s",
                    symbol,
                    period,
                    period,
                )
                ...
                if result == 1:  # Success
                    ...
                    logger.error("Failed to subscribe to %s: %s", symbol, result)
                    return False
    ...
    def unsubscribe_realtime_data(self, symbol: str) -> bool:
        """
        ...
        try:
            with self.subscription_lock:
                if symbol not in self.subscribed_symbols:
                    logger.info("Not subscribed to %s", symbol)
                    return True
                ...
                with self.data_lock:
                    keys_to_remove = [
                        key
                        for key in self.realtime_data_cache
                        if key.startswith(symbol)
                    ]
                    for key in keys_to_remove:
                        del self.realtime_data_cache[key]

                logger.info("Unsubscribed from %s", symbol)
                return True

        except Exception as exc:
            logger.error("Error unsubscribing from %s: %s", symbol, exc)
            return False
    ...
    def get_realtime_data(
        ...
        """
        Get real-time trading data using xtdata.get_market_data_ex.

        Args:
            symbols: Symbol or list of symbols.
            period: Data period.
            field_list: Fields to retrieve (default: OHLCV).

        Returns:
            Dictionary with real-time market data or None on error.
        """
        try:
            if isinstance(symbols, str):
                symbols = [symbols]

            # Ensure subscription for all symbols
            for symbol in symbols:
                if not self.subscribe_realtime_data(symbol, period):
                    logger.warning("Failed to subscribe to %s", symbol)

            if field_list is None:
                field_list = ["open", "high", "low", "close", "volume"]

            logger.info("Getting real-time data for %s", symbols)

            trading_data = xtdata.get_market_data_ex(
            ...
            if trading_data:
                with self.data_lock:
                    for symbol in symbols:
                        self.realtime_data_cache[f"{symbol}_{period}"] = trading_data

                logger.info("Retrieved real-time data for %s", symbols)
                logger.debug("Data structure: %s", type(trading_data))

                if isinstance(trading_data, dict):
                    logger.debug("Data keys: %s", list(trading_data.keys()))

                return trading_data
            else:
                logger.warning("No real-time data received for %s", symbols)
                return None

        except Exception as exc:
            logger.error("Failed to get real-time data: %s", exc)
            return None

    def get_latest_price(self, symbol: str) -> Optional[float]:
        """
        Get the latest price for a symbol via xtdata.get_full_tick.
        ...
        try:
            ...
            if tick_data and symbol in tick_data:
                ...
                # get_full_tick returns a dict with lastPrice (or similar)
                if isinstance(tick, dict):
                    # Try common field names
                    for price_key in ("lastPrice", "last_price", "lastprice"):
                        if price_key in tick:
                            return float(tick[price_key])
                    # Fallback: try lastClose
                    for close_key in ("lastClose", "last_close"):
                        if close_key in tick:
                            return float(tick[close_key])

            # Fallback to real-time OHLCV data
            data = self.get_realtime_data(symbol, field_list=["close"])
            ...
            if data and symbol in data:
                ...
                elif isinstance(symbol_df, dict) and "close" in symbol_df:
                    close_data = symbol_df["close"]
                    if hasattr(close_data, "iloc"):
                        return float(close_data.iloc[-1])

            logger.warning("Could not get latest price for %s", symbol)
            return None

        except Exception as exc:
            logger.error("Error getting latest price for %s: %s", symbol, exc)
            return None

    def get_main_contract_symbol(
        self, underlying: str = "al00.SF"
    ) -> Optional[str]:
        """
        Get the main contract symbol for a futures underlying.

        Args:
            underlying: Underlying symbol (e.g. 'al00.SF').

        Returns:
            Main contract symbol or None on error.
        """
        try:
            logger.info("Getting main contract for %s", underlying)

            main_list = xtdata.get_main_contract(underlying)

            if main_list and len(main_list) > 0:
                logger.info("Main contract for %s: %s", underlying, main_list)
                return main_list
            else:
                logger.warning("No main contract found for %s", underlying)
                return None

        except Exception as exc:
            logger.error("Failed to get main contract: %s", exc)
            return None
    ...
    def query_stock_orders(self) -> List[Dict[str, Any]]:
        """
        Query all stock orders for the account.
        ...
        try:
            ...
            orders = None
            if hasattr(self.trader, "query_stock_orders"):
                orders = self.trader.query_stock_orders(self.account)
                logger.info("query_stock_orders returned: %s", orders)

            if hasattr(self.trader, "query_history_orders"):
                try:
                    hist_orders = self.trader.query_history_orders(self.account)
                    logger.info("query_history_orders returned: %s", hist_orders)
                    if hist_orders and not orders:
                        orders = hist_orders
                except Exception as hist_exc:
                    logger.warning(
                        "query_history_orders failed: %s", hist_exc
                    )

            if not orders:
                logger.info("No orders found with any query method")
                return []
            ...
            for order in orders:
                try:
                    order_dict = {
                        ...
                        "price_type": getattr(order, "price_type", "N/A"),
                        "price": getattr(order, "price", 0.0),
                        "order_status": getattr(order, "order_status", "N/A"),
                    ...
                    logger.error("Error parsing order: %s", parse_exc)
                    logger.error("Order object: %s", order)
                    logger.error("Order attributes: %s", dir(order))

            logger.info("Found %d orders", len(order_list))
            return order_list

        except Exception as exc:
            logger.error("Failed to query stock orders: %s", exc)
            logger.error("Full traceback: %s", traceback.format_exc())
            return []
    ...
    def debug_trader_methods(self) -> Optional[Dict[str, List[str]]]:
        """
        Debug method to list all available methods on the trader object.

        Returns:
            Dictionary with categorized method names or None.
        """
        if not self.trader:
            logger.error("Trader object not initialized")
            return None

        logger.info("=== Available XtQuantTrader methods ===")
        methods = [m for m in dir(self.trader) if not m.startswith("_")]

        position_methods = [m for m in methods if "position" in m.lower()]
        logger.info("Position-related methods: %s", position_methods)

        query_methods = [m for m in methods if m.startswith("query")]
        logger.info("Query methods: %s", query_methods)

        logger.info(
            "Has query_position_statistics: %s",
            hasattr(self.trader, "query_position_statistics"),
        )
        logger.info(
            "Has query_stock_positions: %s",
            hasattr(self.trader, "query_stock_positions"),
        )
